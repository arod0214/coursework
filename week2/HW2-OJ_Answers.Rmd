...
oj %>% mutate(log_price = log(price)) %>%  ggplot(aes(x = "Across all Brands", y = log_price)) + geom_boxplot()
...
oj %>% mutate(log_price = log(price)) %>% ggplot(aes(factor(brand), log_price)) + geom_boxplot(aes(fill = factor(brand)))
...
oj %>% mutate(log_price = log(price)) %>% ggplot(aes(logmove,log_price)) + geom_point(aes(color = factor(brand)))
...
oj %>% mutate(log_price = log(price)) %>% lm(formula = logmove ~ log_price)

#	Now add in an intercept term for each brand (add brand to the regression), how do the results change? 
oj %>% mutate(log_price = log(price)) %>% lm(formula = logmove ~ log_price + brand)
...
train <- oj[sample, ]
test  <- oj[-sample, ]
lm_train <- train %>% mutate(log_price = log(price)) %>% 
  lm(formula = log_price ~ logmove * brand)

oj %>% oj$y_hat <- predict(lm_train, newx = test) %>% ggplot(aes(x = price, y = y_hat), color = brand) + geom_line()
...
oj %>% mutate(log_price = log(price)) %>% ggplot(aes(logmove, log_price)) + geom_point(aes(color = brand), position = "jitter")
...
oj %>% mutate(log_price = log(price)) %>% lm(formula = log_price ~ logmove + feat)
