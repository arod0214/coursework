...
sample <- sample.int(n = nrow(trips_by_weather), size = floor(.8*nrow(trips_by_weather)), replace = F)
train <- trips_by_weather[sample, ]
test  <- trips_by_weather[-sample, ]
...
df1 <- data.frame(train$r_squared_poly_train, test$r_squared_poly_test) %>% head(5)
df1 <- df1 %>% mutate(poly_degree = seq.int(nrow(df1)))
df1 %>% ggplot(aes(x = poly_degree)) + geom_line(aes(y = train.r_squared_poly_train)) + geom_line(aes(y = test.r_squared_poly_test), color = "red")
```
