...
library(tidyverse)
library(readr)
holidays <- read.csv("../week2/US Bank Holidays 2012-2020.csv")
...
reg <- train %>% lm(formula = num_trips~tmin * tmax * prcp * snwd + snow + Bank.Holiday)
summary(reg)
library(modelr)
rmse(reg,test)
rmse(reg,train)
...
# is snow and snwd correlated?
trips_by_weather %>% ggplot(aes(x = snwd + 1,y= snow + 1)) +geom_point(position = "jitter") +scale_x_log10() + scale_y_log10()
# to filter how much snow or snwd you want to see meaured in your model
# I(snwd > 2)
